...
class OSButtons extends React.Component {
    ...
    createSubscribeFields() {
        ...
        const color = '#D45653';
        const elements = [];
        const subscribedButton = renderButtonView(isSubscribed ? "Disable Push" : "Subscribe", color, () => {
            loggingFunction(`Is Push Disabled: ${isSubscribed}`);
            OneSignal.disablePush(isSubscribed);
        });
        const unsubscribeWhenNotificationsAreDisabledButton = renderButtonView(unSubscribedWhenNotificationDisabled ? "Unsubscribe When Notifications Disabled" : "Subscribe when notification disabled", color, () => {
        ...
        const promptForPush = renderButtonView("Prompt for Push", color, () => {
            loggingFunction("Prompting for push with user response...");
            OneSignal.promptForPushNotificationsWithUserResponse(response => {
                loggingFunction(`User response: ${response}`);
            });
        });
        const registerForProvisionalAuthorization = renderButtonView("Register For Provisional Authorization", color, () => {
        ...
        const setLocationShared = renderButtonView(isLocationShared ? "Unshare Location" : "Share Location", color, () => {
        ...
        const promptLocationButton = renderButtonView("Prompt Location", color, () => {
            loggingFunction("Prompting Location");
            OneSignal.promptLocation();
        });
        elements.push(subscribedButton, unsubscribeWhenNotificationsAreDisabledButton, registerForProvisionalAuthorization, locationShared, setLocationShared, promptLocationButton);
        if (Platform.OS === 'ios') {
    ...
    createDeviceFields() {
        ...
        const requireUserProvideConsent = renderButtonView(this.state.requireUserConsent ? "Remove User Privacy Consent Requirement" : "Require User Privacy Consent", color, () => {
        ...
        const appRequireUserProvideConsent = renderButtonView("is Privacy Consent Required", color, async () => {
        ...
        const provideUserConsentButton = renderButtonView(this.state.provideUserConsent ? "Reject User Consent" : "Provide User Consent", color, async () => {
        ...
        elements.push(deviceStateButton, requireUserProvideConsent, appRequireUserProvideConsent, provideUserConsentButton, userProvidedPrivacyConsent);
        return elements;
    }
    createNotificationFields() {
        ...
        const elements = [];
        const { loggingFunction } = this.props;
        const postNotificationButton = renderButtonView("Post Notification", color, async () => {
        ...
        const removeNotificationButton = renderButtonView("Remove Notification With Android ID", color, () => {
            const number = Number(this.props.inputFieldValue);
            loggingFunction("Removing notification with id:", number);
            OneSignal.removeNotification(number);
        });
        const removeGroupedNotificationButton = renderButtonView("Remove Grouped Notifications With Group ID", color, () => {
            const groupId = this.props.inputFieldValue;
            loggingFunction("Removing notification with group id:", groupId);
            OneSignal.removeGroupedNotifications(groupId);
        });
        const sendTagWithKey = renderButtonView("Send tag with key my_tag", color, async () => {
        ...
        elements.push(postNotificationButton, sendTagWithKey, getTags, deleteTagWithKey, removeNotificationButton, removeGroupedNotificationButton);
    ...
    createSMSFields() {
        ...
        const color = "#1E8FEB";
        // Set SMS Button
        const setSMSButton = renderButtonView("Set SMS Number", color, () => {
            loggingFunction('Attempting to set SMS number: ', this.props.inputFieldValue);
            OneSignal.setSMSNumber(this.props.inputFieldValue, undefined, (res) => {
                loggingFunction("setSMSNumber completed with result: ", res);
            });
        });
        // Logout SMS Button
        const logoutSMSButton = renderButtonView("Logout SMS Number", color, () => {
            loggingFunction('Attempting to logout SMS number');
            OneSignal.logoutSMSNumber((res) => {
    ...
    createEmailFields() {
        ...
        const color = "#1E8FEB";
        // Set Email Button
        const setEmailButton = renderButtonView("Set Email", color, () => {
            loggingFunction('Attempting to set email: ', this.props.inputFieldValue);
            OneSignal.setEmail(this.props.inputFieldValue, undefined, (res) => {
                loggingFunction("setEmail completed with result: ", res);
            });
        });
        // Logout Email Button
        const logoutEmailButton = renderButtonView("Logout Email", color, () => {
            loggingFunction('Attempting to logout email');
            OneSignal.logoutEmail((res) => {
                loggingFunction("logoutEmail completed with result: ", res);
            });
        });
        const externalUserIdButton = renderButtonView("Set External User Id", color, () => {
            loggingFunction("Attempting to set external id: ", this.props.inputFieldValue);
            OneSignal.setExternalUserId(this.props.inputFieldValue, "aaa", (res) => {
                loggingFunction("setExternalUserId completed with result: ", JSON.stringify(res));
            });
        });
        const removeExternalIdButton = renderButtonView("Remove External Id", color, () => {
    ...
    createInAppFields() {
        ...
        const { loggingFunction } = this.props;
        const color = "#FEA61D";
        const addTriggerButton = renderButtonView("Add trigger with key my_trigger", color, () => {
        ...
        const removeTriggerButton = renderButtonView("Remove trigger for key", color, () => {
            const key = this.props.inputFieldValue;
            loggingFunction("Removing trigger for key: ", key);
            OneSignal.removeTriggerForKey(key);
        });
        const pauseIamButton = renderButtonView(this.state.pauseIAM ? "Unpause IAM" : "Pause IAM", color, () => {
        ...
        const getTriggerValueForKeyButton = renderButtonView("Get Trigger Value For Key", color, async () => {
            try {
                const key = this.props.inputFieldValue;
    ...
    createOutcomeFields() {
        ...
        const { loggingFunction } = this.props;
        const color = "#FF36A0";
        const sendOutcomeButton = renderButtonView("Send Outcome With Name", color, () => {
            loggingFunction("Sending outcome: ", this.props.inputFieldValue);
            OneSignal.sendOutcome(this.props.inputFieldValue, (event) => {
                loggingFunction("Outcome Event: ", event);
            });
        });
        const sendUniqueOutcomeButton = renderButtonView("Send Unique Outcome With Name", color, () => {
            loggingFunction("Sending unique outcome: ", this.props.inputFieldValue);
            OneSignal.sendUniqueOutcome(this.props.inputFieldValue, (event) => {
                loggingFunction("Unique Outcome Event: ", event);
            });
        });
        const sendOutcomeWithValueButton = renderButtonView("Send Outcome 'my_outcome' with value", color, () => {
            ...
            OneSignal.sendOutcomeWithValue('my_outcome', value, (event) => {
...
//# sourceMappingURL=OSButtons.js.map